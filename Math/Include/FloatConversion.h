...
// fast fp32/fp16/fp10/fp11 handling (will break on NaN/Inf, might have issues with denorm)
namespace fp
{
namespace internal
{
...
inline void ConvertFP32ToFP16(UInt16* __restrict dst, const float* __restrict src, size_t count)
{
    fp::internal::FP32Bits fpbits;
...
inline void ConvertFP16ToFP32(float* __restrict dst, const UInt16* __restrict src, size_t count)
{
    fp::internal::FP32Bits fpbits;
    for (size_t i = 0, n = count; i < n; ++i)
    {
...
private:
    void ConvertImpl(const float& src, UInt16& dest)
    {
        UInt32 bits = AliasAs<UInt32>(src).second;
        UInt8 index = UInt8(bits >> 23);
        UInt32 sign = bits & 0x80000000;
        UInt32 mantissa = bits & 0x007fffff;
        dest = static_cast<UInt16>((sign >> 16) | m_ExponentTable[index] | (mantissa >> m_MantissaShift[index]));
    }

public:
    void Convert(const float& src, UInt16& dest)
    {
        ConvertImpl(src, dest);
    }

    void Convert(size_t count, const float* src, UInt16* dest)
    {
        for (size_t i = 0, n = count; i < n; ++i)
            ConvertImpl(src[i], dest[i]);
    }

    static void InitializePrecomputeTables(void*);

private:
    static UInt16 m_ExponentTable[256];
    static UInt8 m_MantissaShift[256];
};

extern FloatToHalfConverter g_FloatToHalf;

inline UInt16 FloatToHalf(float src)
{
    UInt16 dest;
...
// TODO: match interface to ConvertFP32ToFP16
inline void HalfToFloat(UInt16 src, float& dest)
{
